...
Before going to the details, Some notations are presented first. $\mathbf{R}_c$, $\mathbf{R}_v$, $\mathbf{R}_r$ are the rotations of the camera, the vehicle and the road in world coordinate frame. $\mathbf{R}_{rc}$ means the rotation of the road relative in camera coordinate frame. Others, such as $\mathbf{R}_{cv}$, have similar meanings. $\mathbf{R}_{c,k}$ means the rotation of the camera in frame $k$. $s_{c,k}$ means the position of the camera in frame $k$. $\mathbf{e}_1=(1,0,0)^T$, $\mathbf{e}_2=(0,1,0)^T$, $\mathbf{e}_3=(0,0,1)^T$ are the directions of x,y and z in world coordinate frame. $\mathbf{P}=\mathbf{K}[\mathbf{R}|\mathbf{t}]$ is the projection matrix of the camera, where $\mathbf{K}$ is the intrinsic matrix, which is well calibrated, $\mathbf{R}$ is the rotation matrix, and $\mathbf{t}$ is the translation vector.

\subsection{Map-based Prediction}
The pose of the vehicle ($\hat{\mathbf{x}}_k = \{\hat{\mathbf{R}}_{v,k}, \hat{\mathbf{s}}_{v,k}\}$) in frame $k$ is predicted by a vehicle motion model with nonholonomic constraint. In this model, the change of the rotation is assumed to be linear, which means
\begin{equation}
...
After that, the pose of the camera is calculated using the $4\times4$ calibration matrix $\mathbf{M}_{cv}$, which means

\begin{equation}
\begin{bmatrix} \hat{\mathbf{R}}_{c,k}^T \\ \hat{\mathbf{s}}_{c,k}^T\end{bmatrix}
=
\mathbf{M}_{cv}^T
\cdot
\begin{bmatrix} \hat{\mathbf{R}}_{v,k}^T \\ \hat{\mathbf{s}}_{v,k}^T\end{bmatrix}
\end{equation}

Then the camera's field of view is projected onto the map to get the direction of the road in a straight road or directions of the roads in the intersection. For the intersection situation, the road is back projected onto the image and divide the image into several zones for line classification as shown in Fig. \ref{fig:map_based_prediction}.
...
Suppose the line segments $\mathbf{l}_i$ matches with rotation $\mathbf{R}_r^i$, the directions of road's longitude, latitude and vertical on the world coordinate are $\mathbf{R}_r^i\mathbf{e}_j,(j=1,2,3)$. So the directions on the camera coordinate are $\mathbf{R}_c^T\mathbf{R}_r^i\mathbf{e}_j,(j=1,2,3)$.
...
Using the predicted rotation $\hat{\mathbf{R}}_c$ and the distance measurement, the line segments are selected given a threshold on the distance measure. For the selected line segments, there directions are given by the road's direction with minimum distance.
\begin{equation}
...
As shown in (\ref{eq:C_RSF}), a RSF candidate is defined using three line segments and two points. Suppose the line segments are $\mathbf{l}_1,\mathbf{l}_2,\mathbf{l}_3$ with their classified directions $\hat{\mathbf{d}}_1,\hat{\mathbf{d}}_2,\hat{\mathbf{d}}_3$ ($\hat{\mathbf{d}}_1=\hat{\mathbf{d}}_2,\hat{\mathbf{d}}_1\neq\hat{\mathbf{d}}_3$). $\mathbf{p}_1, \mathbf{p}_2$ are the feature points and $\mathbf{p}_1^\prime, \mathbf{p}_2^\prime$ are the tracked points in the previous frame. The rotation and position of the camera are $\mathbf{R}_c$ and $\mathbf{s}_c$. $\mathbf{P}=\mathbf{K}[\mathbf{R}|\mathbf{t}]$ is the projection matrix of the camera. Here $\mathbf{R}=\mathbf{R}_c^T$,
$\mathbf{t} = -\mathbf{R}_c^T\mathbf{s}_c$.
